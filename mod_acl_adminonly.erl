...
observe_acl_is_allowed(#acl_is_allowed{}, #context{user_id = UserId}) when UserId =/= undefined ->
    true;
...
observe_acl_is_allowed(#acl_is_allowed{}, #context{}) ->
	false.
...
%% @doc Limit what anonymous users can find
observe_acl_add_sql_check(#acl_add_sql_check{alias=Alias, args=Args, search_sql=SearchSql}, #context{user_id = undefined}) ->
    SQL = case lists:member(no_publish_check, SearchSql#search_sql.extra) of
        true ->
            "";
        false ->
            " and "
    ...
    {"visible_for = 0 "++SQL, Args};
observe_acl_add_sql_check(#acl_add_sql_check{args=Args}, _Context) ->
    {[], Args}.
